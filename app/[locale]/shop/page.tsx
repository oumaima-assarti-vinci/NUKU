...
type DbProduct = {
  ...
  usage_instructions?: string | null; gummies_per_jar?: number | null; flavor?: string | null; shipping_note?: string | null;
};
...
function normalizeImagesField(images: DbProduct["images"]): string[] {
  try {
    ...
    if (typeof images === "string") {
      ...
      if (trimmed.includes(",")) return trimmed.split(",").map((s) => s.trim()).filter((x) => x.length > 0);
...
function getFirstImage(images: DbProduct["images"]): string {
  ...
  return list[0] ?? "https://images.unsplash.com/photo-1556228852-80c63843f03c?w=800&auto=format&fit=crop";
}
...
function getProductGlowColor(name: string) {
  ...
  if (n.includes("STRENGTH") || n.includes("STRENGHT")) return { color: "rgba(255,80,80,1)", bg: "rgba(255,80,80,0.12)" };
...
function BenefitsNoMiddle({ benefits }: { benefits: string[] }) {
  ...
  return (
    <div className="mt-4">
      {rows.map((row, idx) => (
        <div key={idx} className={`grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4 py-5 ${idx !== 0 ? "border-t border-neutral-200" : ""}`}>
          {row.map((text, i) => (
            <div key={i} className="flex items-center gap-4">
              <span className="text-xl md:text-2xl leading-none select-none">{getBenefitEmoji(text)}</span>
...
function ProductLabel({ title }: { title: string }) {
  return (
    <div className="mt-4 text-center">
      <h3 className="text-xs md:text-sm font-light tracking-[0.2em] text-neutral-400 uppercase">{title}</h3>
    </div>
  );
}

function ProductCard({ product, onClickProduct }: { product: DbProduct; onClickProduct: (id: number) => void; }) {
  const [isHovered, setIsHovered] = useState(false);
  const img = getFirstImage(product.images);
  const { color } = getProductGlowColor(product.nom);
  ...
  return (
    <div className="relative flex flex-col items-center cursor-pointer group"
      ...
      <div className="relative z-10 w-[110px] h-[110px] sm:w-[140px] sm:h-[140px] md:w-[180px] md:h-[180px] flex items-center justify-center">
        <motion.img src={img} alt={product.nom} className="w-full h-full object-contain" style={{ willChange: "filter, transform" }}
          animate={{ scale: isHovered ? 1.1 : 1, y: isHovered ? -8 : 0,
            filter: isHovered
          ...
          onError={(e) => { (e.currentTarget as HTMLImageElement).src = "https://images.unsplash.com/photo-1556228852-80c63843f03c?w=800&auto=format&fit=crop"; }} />
      </div>
    </div>
  );
}
...
export default function ShopPage() {
  ...
  const handleProductClick = (productId: number) => { router.push(`/product/${productId}`); };
  ...
  useEffect(() => {
    (async () => {
      ...
      (summaries as ReviewSummary[] | null)?.forEach((r) => { map[r.product_id] = { avg: Number(r.rating_avg ?? 0), count: Number(r.reviews_count ?? 0) }; });
      setSummaryMap(map);
      setLoading(false);
    })();
  }, []);
    ...
    <div className="bg-white text-neutral-900 min-h-screen pt-[73px]">
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="flex justify-center items-end gap-3 sm:gap-8 md:gap-20 mb-8">
            {[products[0], products[1], products[2]].map((p, i) =>
              p ? (
                <motion.div key={p.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: i * 0.1 }} className="flex flex-col items-center">
          ...
          <div className="flex justify-center items-end gap-3 sm:gap-8 md:gap-20">
            {[products[3], products[4]].map((p, i) =>
              p ? (
                <motion.div key={p.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 + i * 0.1 }} className="flex flex-col items-center">
                  <ProductCard product={p} onClickProduct={handleProductClick} />
                  <ProductLabel title={p.nom} />
                </motion.div>
              ) : null
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.3 }}
              ...
              <div className="flex items-center justify-between p-6 border-b border-neutral-200">
                ...
                <button onClick={() => setSelectedProduct(null)} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-neutral-100 transition-colors">
                ...
                <nav className="flex items-center gap-6">
                  {(["details", "reviews"] as const).map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      className={`py-3 font-semibold border-b-2 transition-colors capitalize ${activeTab === tab ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-500 hover:text-neutral-800"}`}>
                      {tab === "details" ? t("details") : t("reviews")}
                    </button>
                  ))}
                </nav>
              </div>
                ...
                {activeTab === "details" ? (
                  <div className="grid md:grid-cols-2 gap-8 p-6">
                    <div className="bg-neutral-100 rounded-2xl aspect-square flex items-center justify-center p-12">
                      <img src={getFirstImage(selectedProduct.images)} className="h-[60%] object-contain drop-shadow-2xl" alt={selectedProduct.nom} />
                    </div>
                    <div className="space-y-6">
                      {selectedProduct.story && <p className="text-neutral-700 leading-relaxed">{selectedProduct.story}</p>}
                      {(selectedProduct.benefits?.length ?? 0) > 0 && (
                        <div>
                          <h3 className="text-lg font-semibold mb-1">{t("benefits")}</h3>
                          <BenefitsNoMiddle benefits={selectedProduct.benefits!} />
                        </div>
                      )}
                      <div className="p-6 bg-neutral-50 rounded-2xl">
                        <div className="flex items-baseline gap-2 mb-4">
                          <span className="text-4xl font-bold">{priceLabel(selectedProduct, purchaseType)}€</span>
                          <span className="text-sm text-neutral-500">{purchaseType === "subscription" ? t("per_month") : t("unit_price")}</span>
                        </div>
                      </div>
                      <div className="space-y-3">
                        {(["unique", "subscription"] as const).map((type) => (
                          <label key={type}
                            className={`flex items-center justify-between p-5 border-2 rounded-2xl cursor-pointer transition-colors ${purchaseType === type ? type === "unique" ? "border-neutral-900 bg-neutral-50" : "border-green-500 bg-green-50" : "border-neutral-200"}`}>
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-semibold">{type === "unique" ? t("unique") : t("subscription")}</span>
                                {type === "subscription" && <span className="px-2 py-0.5 bg-green-500 text-white text-xs font-bold rounded">-20%</span>}
                              </div>
                              <span className="text-sm text-neutral-600">{type === "unique" ? t("unique_subtitle") : t("subscription_subtitle")}</span>
                            </div>
                            <input type="radio" checked={purchaseType === type} onChange={() => setPurchaseType(type)} className="w-5 h-5" />
                          </label>
                        ))}
                      </div>
                      <button
                        onClick={() => {
                          addToCart({ id: selectedProduct.id, nom: selectedProduct.nom, prix: selectedProduct.prix, images: normalizeImagesField(selectedProduct.images) } as any, purchaseType === "subscription");
                        ...
                        {t("add_to_cart")} — {priceLabel(selectedProduct, purchaseType)}€
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="p-6"><ProductReviews productId={selectedProduct.id} /></div>
                )}
              </div>
            </motion.div>
          </motion.div>
